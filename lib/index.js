    ...
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        if (cfg.githubSecret) {
          ...
          if (!signature) {
            logger.warn("github-review: webhook request missing signature header");
            res.writeHead(401, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "Missing signature" }));
            return;
          }
          if (!verifyWebhookSignature(cfg.githubSecret, signature, body)) {
            logger.warn("github-review: webhook signature verification failed");
            res.writeHead(403, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ error: "Invalid signature" }));
            return;
          }
          logger.debug("github-review: webhook signature verified");
        }
